...
def configura_ini():
	...
	return {'email': arquivo.get('GERAL', 'email'),'senha': arquivo.get('GERAL', 'senha'), 'modo': arquivo.get('GERAL', 'modo'),'porcento': arquivo.get('GERAL', 'porcento')}

conf = configura_ini()
API = IQ_Option(str(conf['email']), str(conf['senha']))
API.connect()

API.change_balance(str(conf['modo'])) # PRACTICE / REAL
...
def entradas(config, entrada, direcao, timeframe, paridade):
...
def actives():
	...
	try:
		for paridade in pares['digital']:
			if pares['digital'][paridade]['open'] == True and paridade != 'NZDUSD':
				selecionados.append(paridade)
		
	except Exception as e:
		selecionados.append(config['paridade'])
...
def main():
	...
	while True:
		for timefr in list_times:
			timeframe = 'PT'+timefr+'M' # PT5M / PT15M
			old = 0
			# config['valor_minimo'] = config['valor_minimo']*5
			# Captura o Payout

			#config['valor_entrada']  = banca()*float(conf['porcento'])

			for paridade in actives():
				config['payout'] = float(payout(paridade, 'digital', int(timefr)) / 100)
				...
				if len(trades) > 0 and old != trades[0]['user_id'] and trades[0]['amount_enrolled'] >= float(config['valor_minimo']):
					...
					if ok:
						...
						config['valor_entrada'] = float(trades[0]['amount_enrolled'])
						# 1 entrada
						resultado,lucro,stop = entradas(config, config['valor_entrada'], trades[0]['instrument_dir'], int(timefr), paridade)
						...
						if resultado == 'loss' and config['martingale'] == 'S':
							...
							for i in range(int(config['niveis']) if int(config['niveis']) > 0 else 1):
								...
								resultado,lucro,stop = entradas(config, valor_entrada, trades[0]['instrument_dir'], int(timefr), paridade)
								print(' ',resultado,'/',lucro,'\n')
								if stop:
									print('\n\nStop',resultado.upper(),'batido!')
									sys.exit()
							...
							if float(config['valor_entrada']) > 5:
								...
								for i in range(int(config['niveis']) if int(config['niveis']) > 0 else 1):
									...
									for i2 in range(2):
										...
										# Entrada
										resultado,lucro,stop = entradas(config, (perca / 2)+lucro, trades[0]['instrument_dir'], int(timefr), paridade)							
										print(resultado,'/',lucro,'\n')	
										if stop:
											print('\n\nStop',resultado.upper(),'batido!')
											sys.exit()
								...
								if lucro>0:
									...
									derrotas+=1
								placar = {'vitorias':  vitorias, 'derrotas': derrotas}
								print(placar)
